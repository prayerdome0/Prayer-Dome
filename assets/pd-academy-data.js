...
(function () {
  ...
  var tracks = [
    { id: 'foundations', title: 'Foundations', icon: 'pd-i-latin-cross', color: 'blue', summary: 'Know what you believe and why it matters.' },
    { id: 'prayer', title: 'Prayer & Intercession', icon: 'pd-i-hands-praying', color: 'gold', summary: 'Grow in daily prayer, night prayer and spiritual warfare.' },
    { id: 'word', title: 'The Word of God', icon: 'pd-i-book', color: 'blue', summary: 'Study, meditate, obey and teach Scripture with confidence.' },
    { id: 'spirit', title: 'Holy Spirit & Gifts', icon: 'pd-i-flame', color: 'gold', summary: 'Walk in the Spirit and serve with His power and character.' },
    { id: 'character', title: 'Christlike Character', icon: 'pd-i-heart', color: 'blue', summary: 'Develop faith, integrity, patience, stewardship and love.' },
    { id: 'mission', title: 'Mission & Service', icon: 'pd-i-earth', color: 'gold', summary: 'Become active in evangelism, fellowship, giving and ministry.' }
  ];
  ...
  // Lesson-specific question pools. Track banks give every quiz its subject
  // grounding; these pools make each lesson's knowledge check genuinely about
  // what that lesson taught, so attempts vary and stay meaningful.
  ...
  var quizzes = lessons.map(function (lesson) {
    var questions = [];
    // 6 subject questions drawn from the track bank (see questionForLesson).
    for (var i = 0; i < 6; i++) questions.push(questionForLesson(lesson, i));
    // 4 lesson-specific questions, so every knowledge check is really about
    // the lesson just studied and repeated attempts see fresh questions.
    var specific = LESSON_QUESTIONS[lesson.id] || [];
    specific.forEach(function (q) { questions.push(q); });
